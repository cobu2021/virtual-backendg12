...
class IngredienteController(Resource):
    def get(self, id):
        #filter_by > tenemos que indicar dentro de este metodo las columnas que
        #queremos usar para hacer el filtro con su respectivo valor
        #el parametro sera el nombre del atributo definido en el modelo y el segundo
        #sera el valor
        #SELECT TOP 1 * FROM ingredientes WHERE id= $id
        ingrediente = conexion.session.query(Ingrediente).filter_by(id=id).first()
        print(ingrediente)
        if ingrediente:
            #mando llamar a mi DTO de respuesta de ingrediente
    ...
    def put(self, id):
        ...
        try:
            if ingrediente:
                ...
                #validamos la data enviada por el usuario para que cumpla con lo definido
                #en el DTO
                data_validada = IngredienteRequestDTO().load(body)
                #al ya validar nuestro ingrediente y que este exista procedemos a modificar
                #sus columnas(solo seria nombre) con el nuevo valor enviado por el usuario
                #previamente ya validado
                ingrediente.nombre = data_validada.get('nombre')
                #solamente hacemos un commit ya que no estamos agregando nuevos valores a la
                #base de datos
                conexion.session.commit()
                #Usando el DTO de response pasar el ingrediente y que me devuelva su informacion
                #para agregarla en el content de la respuesta
                resultado = IngredienteResponseDTO().dump(ingrediente)
                return {
                    'message' : 'Ingrediente actualizado exitosamente',
                    'content' : resultado
                }
            #que pasa si el ingrediente no existe O si ka data es incorrecta? 
            #solucionar esos errores
             ...
             return {
                    'message' : 'Error al actuliazar el ingrediente',
                    'content' : e.args
                }, 400
