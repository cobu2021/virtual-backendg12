...
def generar_comprobante(tipo_de_comprobante: int, tipo_documento: str,numero_documento:str, pedido_id:int):
    """Sirve para generar un comprobante electronico ya sea factura,Boleta o
    notas en base a un pedido"""
    ...
    #esta serie se deberia de guardar en la bd en una tabla de series para que
    #el comtador desee cambiar de serie se modifique en esa tabla
        serie = 'FFF1'
        tipo = 'FACTURA'
    ...
    # si queremos hacer uso de una vista,vista materializada, funcion o procedimiento
    #almacenado que no este registrado en el ORM entonces podremos hacer uso de una
    #RAW QUERY de la siguiente manera :
    with connection.cursor() as cursor:
         cursor.execute('SELECT * FROM vista_prueba')
         print(cursor.fetchall())

    # lo sacaremos del ultimo comprobante almacenado en la bd
    #select numero, serie form comprobante where tipo = 'boleta' / 'factura' order by numero desc LIMIT 1;
    ...
    if tipo_documento is None:
        ...
        if tipo_documento == 'RUC' :
            respuesta_apiperu = requests.get("http://apiperu.dev/api/ruc/"+numero_documento,
            ...
            if respuesta_apiperu.json().get('sucess') == False:
                raise Exception('Datos del Cliente no Validos')
            else:
                cliente_denominacion= respuesta_apiperu.json().get('nombre_completo')
                cliente_direccion = ' '
    cliente_email = 'luis9carbajal9aguilar@gmail.com'
    ...
    #el valor sin igv (la base imponible)
    ...
    for detalle in detalle_pedido:
        ...
        #Estraigo el stock de ese detalle
